package org.example.gatewayservice.handler;
...
/**
 * 全局错误处理器，用于统一处理网关中的异常并返回标准化的错误响应。
 * 该类继承自 Spring Boot 的 AbstractErrorWebExceptionHandler 类，并重写了相关方法，以实现自定义错误响应的逻辑。
 * @author moqi
 */
@Component
...
public class GlobalExceptionHandler extends AbstractErrorWebExceptionHandler {
    ...
    private Mono<ServerResponse> renderErrorResponse(final ServerRequest request) {
        ...
        // 识别时间阻断错误
        if (errorMap.containsKey("TIME_BLOCKED")) {
            errorMap.put("code", 403);
            errorMap.put("msg", "当前时间段禁止访问");
